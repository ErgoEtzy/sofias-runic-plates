---
title: "Runological method"
note_type: "concept"
source: "Sofia Pereswetoff-Morath - Viking-Age Runic Plates"
source_printed_page: 34
tags: ["runes/concept", "sofia-pereswetoff-morath"]
---

# Runological method

## In the book

The method begins with the object: repeated examination under a stereomicroscope, movable lighting, photographs, drawings, and comparison with other inscriptions. A proposed interpretation should follow the reading rather than control it.

[Book p. 34](../Sofia_Pereswetoff_Morath_Viking_Age_Runic_Plates_Readings_and_Interpretations.pdf#page=36) / [book note](<../Sofia Pereswetoff-Morath - Viking-Age Runic Plates.md>)

## Wider context

Runology combines epigraphy, historical linguistics, archaeology, and object study. Strong interpretations make each step visible: what is carved, how it is transliterated, how words are divided, which language forms are proposed, and where uncertainty remains.

## See also

[[Reading and interpretation]] / [[Transliteration and normalization]] / [[Rune-like markings]] / [[Dating runic inscriptions]]

## Examples

- [Kvinneby amulet: deciphering attempts](https://en.wikipedia.org/wiki/Kvinneby_amulet#Deciphering_attempts). Several readings laid out far enough to compare their assumptions.
- [Vg 8: inscription](https://en.wikipedia.org/wiki/V%C3%A4sterg%C3%B6tland_Runic_Inscription_8#Inscription). The standard text layers used in runology.

## Further reading

- [Wikipedia: Runology](https://en.wikipedia.org/wiki/Runology)
- [Wikipedia: History of runology](https://en.wikipedia.org/wiki/Runes#Runology)
- [Uppsala University: Runology](https://www.uu.se/en/study/course?query=5SV125)
- [Swedish National Heritage Board: Reading runic inscriptions](https://www.raa.se/kulturarv/runor-och-runstenar/att-lasa-runor-och-runinskrifter/)
